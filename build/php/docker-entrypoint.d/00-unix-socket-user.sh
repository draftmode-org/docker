...
entrypoint_log "[NOTICE] ${0}: create GROUP (${PHP_FPM_LISTEN_GROUP} ${PHP_FPM_LISTEN_GROUP_ID}) for shared socket access"
# shellcheck disable=SC2046
[ $(getent group ${PHP_FPM_LISTEN_GROUP}) ] || addgroup -g ${PHP_FPM_LISTEN_GROUP_ID} -S ${PHP_FPM_LISTEN_GROUP}

entrypoint_log "[NOTICE] ${0}: add user ($(whoami)) to group ${PHP_FPM_LISTEN_GROUP}"
